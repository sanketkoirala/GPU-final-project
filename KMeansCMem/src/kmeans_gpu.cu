...
// Phase 2: Constant Memory for Centroids.
// Centroids are read by every thread in assign_kernel → store them in
// __constant__ memory, which is cached and broadcast to warps.
// This eliminates global-memory reads for centroids (~50% DRAM traffic drop).
// Update kernel is unchanged from Phase 1.

__constant__ float kCentroids_d[MAX_K * MAX_D];

__global__ void assign_kernel(const float *data, int *assignments,
                              int N, int D, int K) {
    ...
    for (int k = 0; k < K; ++k) {
        float dist = 0.0f;
        for (int d = 0; d < D; ++d) {
            float diff = data[i * D + d] - kCentroids_d[k * D + d];
            dist += diff * diff;
        }
        if (dist < minDist) {
            minDist = dist;
...
void kmeansGPU(const float *points, const float *initCentroids,
    ...
    for (int t = 0; t < T; ++t) {
        // Copy current centroids to constant memory for the assign kernel
        gpuErrchk(cudaMemcpyToSymbol(kCentroids_d, centroids_d,
                                     centroidsBytes, 0,
                                     cudaMemcpyDeviceToDevice));

        // Assign each point to nearest centroid (reads centroids from __constant__)
